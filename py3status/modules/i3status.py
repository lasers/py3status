...
i3status {
    ...
    format_module_separator = True # enable separator
}
...
class Py3status:
    ...
    def _write_i3status_config(self):
        ...
        for name in ("bad", "degraded", "good"):
            color = getattr(self.py3, f"COLOR_{name.upper()}")
            if self.py3.is_color(color):
                self.general.setdefault(f"color_{name}", color)

        markup = self.py3.get_config("markup")
        if markup:
            self.general.setdefault("markup", markup)

        lines = []
        excluded = ("name", "format_module", "interval")
        ...
        for module in self.modules:
            ...
            settings = {k: v for k, v in module.items() if k not in excluded}
            if not settings:
                continue
        ...
        try:
            # python 3.12+
            self.tmpfile = NamedTemporaryFile(mode="w", encoding="utf-8", prefix="py3status-i3status_",
                suffix=".conf", delete_on_close=False)
        except TypeError:
            self.tmpfile = NamedTemporaryFile(mode="w", encoding="utf-8", prefix="py3status-i3status_",
                suffix=".conf", delete=False)
        # fmt: on
        self.tmpfile.write("".join(lines))
        self.tmpfile.flush()
        self.tmpfile.close()
    ...
    def _find_module(self, item):
        ...
        runtime = {"key": key, "time": 0, "output": None}
    ...
    def _format_modules(self):
        ...
        for item in self.output:
            ...
            if runtime["output"] is None or interval is None or now - runtime["time"] >= interval:
                if interval and runtime["output"] is not None:
    ...
    def i3status(self):
        ...
        format_module = self.py3.safe_join(self.format_module_separator, self._format_modules())
...
if __name__ == "__main__":
    ...
    config = {
        "modules": [
            ...
            {"name": "battery all", "format": "BAT %status", "format_down": "", "status_bat": ""},
